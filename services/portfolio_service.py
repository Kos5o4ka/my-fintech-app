...
def build_portfolio_list(active_bonds: list[BondPortfolio]) -> tuple[list[dict], float]:
    """Возвращает (список позиций, суммарная стоимость портфеля)."""
...
def calc_portfolio_ytm(portfolio_list: list[dict], total_value: float) -> float:
    """Средневзвешенная YTM портфеля (вес = стоимость позиции)."""
    ytm_weight_sum = sum(b["ytm"] * b["current_value"] for b in portfolio_list if b["ytm"])
    return round(ytm_weight_sum / total_value, 2) if total_value else 0.0


def build_trade_entry(bond: BondPortfolio) -> dict:
...
def calc_coupon_income(active_bonds: list[BondPortfolio]) -> dict[str, float]:
    """Прогноз купонного дохода в разрезах 30/90/365 дней."""
    today = date.today()
...
def calc_monthly_profit(closed_bonds: list[BondPortfolio]) -> dict[str, float]:
    """Ежемесячная зафиксированная прибыль по закрытым позициям."""
    ...
    for bond in closed_bonds:
        ...
        monthly[group_date.strftime("%Y-%m")] += (sell_p - float(bond.buy_price)) * bond.amount
    return dict(monthly)
...
def calc_tax_report(
    ...
    """Расчёт НДФЛ 13% с дохода от сделок и купонных выплат за год."""
