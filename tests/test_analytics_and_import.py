...
class AnalyticsAndImportTests(BaseTest):
    ...
    def test_portfolio_sharpe_ratio_success(self):
        ...
        self._make_bond(
            uid, isin="ISIN1", is_sold=True, buy_price=100.0, sell_price=110.0, amount=1
        )
        self._make_bond(
            uid, isin="ISIN2", is_sold=True, buy_price=100.0, sell_price=120.0, amount=1
        )
        self._make_bond(
            uid, isin="ISIN3", is_sold=True, buy_price=100.0, sell_price=115.0, amount=1
        )
    ...
    def test_compare_bonds_invalid_params(self):
        ...
        # Identical params
        r = self.client.get(
            "/api/portfolio/compare?isin1=SU26238RMFS4&isin2=SU26238RMFS4"
        )
        self.assertEqual(r.status_code, 400)
    ...
    def test_dashboard_pnl_chart(self):
        ...
        # Make one active lot
        self._make_bond(
            uid, isin="SU26238RMFS4", is_sold=False, buy_price=900.0, amount=5
        )
    ...
    def test_circuit_breaker_failsafe(self):
        ...
        with patch("app.extensions.cache.get", side_effect=mock_get), patch(
            "app.extensions.cache.set", side_effect=mock_set
        ), patch("app.extensions.cache.delete", side_effect=mock_delete), patch(
            "app.moex.requests.get",
    ...
    def test_profile_api_stats(self):
        ...
        self._make_bond(
            uid, isin="SU26238RMFS4", is_sold=False, buy_price=900.0, amount=10
        )
        self._make_bond(
            uid,
            isin="RU000A105C94",
