// Watchlist + Screener — грузится лениво при первом клике по соответствующим элементам.
// portfolio.js устанавливает заглушки, которые при первом вызове подгружают этот файл.
...
(function () {
    ...
    function _skeletonRows(cols, rows) {
        ...
        for (let r = 0; r < rows; r++) {
            html += '<tr>';
            for (let c = 0; c < cols; c++) html += '<td><span class="skeleton-cell" style="width:60%"></span></td>';
            html += '</tr>';
        }
        return html;
    }
    ...
    window.loadWatchlistIfNeeded = function () {
        if (!watchlistLoaded) fetchWatchlist();
    };

    window.addWatchlistDirect = async function () {
    ...
    window.removeFromWatchlist = async function (isin) {
    ...
    window.runScreener = async function () {
        ...
        const ids = {
            min_ytm: 'screenerMinYtm', max_ytm: 'screenerMaxYtm',
            maturity_from: 'screenerMatFrom', maturity_to: 'screenerMatTo',
            issuer_type: 'screenerIssuerType',
            min_duration: 'screenerMinDuration', max_duration: 'screenerMaxDuration',
        };
    ...
    window.addScreenerToWatchlist = async function (isin) {
        const res = await window.Common.csrfFetch('/api/watchlist', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    window.__portfolioExtrasLoaded = true;
})();
