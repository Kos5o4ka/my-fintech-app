...
> **Ветка активной разработки:** `feature/architecture-optimization`
...
| 10 | Мультиключевой импорт и Валюты (v2.0.0) | ⏳ Запланирован |
...
## Этап 10 — Мультиключевой импорт и Валюты (v2.0.0) ⏳ ЗАПЛАНИРОВАН

> Цель: Поддержка замещающих валютных облигаций, золотых бумаг и максимальное упрощение ввода активов.

### 10.1 Улучшение юзабилити и автозаполнение цен
- [ ] **Быстрый импорт**: При вводе ISIN в форму «Добавить облигацию» автоматически подтягивать текущую цену MOEX ISS в поле ввода цены покупки по умолчанию.
- [ ] **Импорт в 1 клик**: Добавить чекбокс «Импортировать по рыночной цене» для пропуска ручного ввода котировки.
- [ ] **Массовый импорт**: Поддержка добавления списка ISIN через запятую / текстовое поле (авто-распределение).

### 10.2 Валютные облигации (CNY, USD, EUR)
- [ ] **Мультивалютная БД**: Добавление поддержки разных валют номинала (`currency` в модели `BondPortfolio`).
- [ ] **Замещающие CNY-облигации**: Поддержка бумаг, номинированных в юанях (например, Роснефть, Полюс, Металлоинвест).
- [ ] **Динамическая конвертация**: Парсинг курсов валют USDRUB и CNYRUB с валютной секции Московской Биржи в реальном времени.
- [ ] **Валютная переоценка**: Пересчёт стоимости всего портфеля и P&L in RUB по актуальному курсу.
- [ ] **Валютный дашборд**: Возможность переключения валюты отображения всего личного кабинета (RUB / USD / CNY) для контроля активов в твёрдой валюте.

### 10.3 «Золотые» облигации (GLD)
- [ ] **Золотой номинал**: Интеграция котировок золота Мосбиржи (`GLDRUB_TOM`) для динамического расчёта стоимости облигаций, номинированных в золоте (например, выпуски Селигдара SELG01, Полюса PLZL01).
- [ ] **Амортизация в золоте**: Корректный пересчёт выплат по купонам и амортизациям с привязкой к весу драгметалла.

### 10.4 Интеграция с брокерскими отчетами
- [ ] **Парсинг PDF/XLSX**: Добавление модуля импорта брокерских отчетов (Т-Банк, ВТБ, Сбер, Финам) для автоматической загрузки всей истории сделок и позиций без ручного ввода.
