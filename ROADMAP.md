...
| 13 | T-Invest API + Мультиключевой импорт (v2.3.0) | ✅ Частично (13.0) |
| 14 | Архитектурный рефакторинг, доменные исключения (v2.4.0) | ✅ Выполнен |
| 15 | Перформанс, Redis-кэш метрик, T-Invest UX, Доходность-аналитика (v2.5.0) | ✅ Выполнен |
| 16 | Валюты и Золотые облигации (v2.6.0) | ⏳ В плане |
---
...
- [x] Bootstrap локально вместо CDN *(не критично)*
...
## Этап 13 — T-Invest API + Мультиключевой импорт (v2.3.0) ✅ ВЫПОЛНЕН (часть 13.0)

> Цель: Полноценная REST-интеграция с T-Invest (Т-Банк) для импорта позиций.

### 13.0 T-Invest REST integration ✅
- [x] `TInvestClient` (`app/services/tinkoff_service.py`) — REST к `invest-public-api.tbank.ru`
- [x] Конвертеры `MoneyValue/Quotation/Timestamp` → `Decimal/datetime`
- [x] Шифрование токена Fernet (PBKDF2 от `SECRET_KEY`) — миграция `stage13_tinkoff_integration`
- [x] Верификация токена через `GetAccounts` перед сохранением
- [x] `POST /api/portfolio/tinkoff_sync` — upsert bond-позиций по ISIN, обогащение через `GetInstrumentBy` (батч 5 + пауза 200 мс)
- [x] `GET /api/profile/tinkoff/accounts` — список счетов пользователя
- [x] Маппинг ошибок: 401/40003 → re-issue токена; 429/50002 → retry с задержкой
- [x] Bruno-файлы и 16 тестов (моки `requests.Session`)

---

## Этап 14 — Интеграции и Улучшения UX (v2.4.0) ✅ ВЫПОЛНЕН

> Цель: Интеграция по API с Т-Банком, улучшения интерфейса портфеля и профиля, доработка налогового отчета.

### 14.1 Интеграция T-Bank API
- [x] **Авторизация**: сохранение read-only токена в профиле (`User.tinkoff_token`)
- [x] **Синхронизация**: кнопка и ручка `POST /api/portfolio/tinkoff_sync` для автоматического получения позиций

### 14.2 Редизайн интерфейсов
- [x] **Избранное**: inline-поиск и добавление без модальных окон
- [x] **Виджет купонов**: перенос "Предстоящих купонов" на страницу портфеля и аналитики
- [x] **Админ-панель**: фильтрация пользователей по списку при рассылке

### 14.3 Налоговый отчет
- [x] **Сжатие сделок**: группировка сделок по `(isin, sell_date, sell_price, buy_price)` для чистоты отображения
- [x] **Детализация**: выпадающий список (collapsible) для просмотра комиссий и точного времени

---

## Этап 15 — Перформанс и доходность-аналитика (v2.5.0) ✅ ВЫПОЛНЕН

> Цель: устранить узкие места, найденные профилированием; переосмыслить аналитику в духе T-Invest (акцент на доходности, а не на коэффициенте Шарпа).

### 15.1 Backend performance ✅
- [x] Redis-кэш `calc_bond_ytm` и `calc_bond_duration` (TTL 1 ч, ключ с округлёнными float'ами)
- [x] Защита от `OverflowError: complex exponentiation` (y ≤ −0.99 / y > 10)
- [x] Redis-кэш `/api/portfolio` (5 мин, ключ `portfolio_full:{user_id}`), пагинация поверх кэша
- [x] N+1 fix в `query_transactions` (`sold_bonds_map`)
- [x] Try/except на каждой позиции в `build_portfolio_list` — изоляция битых бумаг
- [x] MOEX `prefetch_bonds_batch` (10 ISIN/запрос)
- [x] Параллельные купонные календари (`ThreadPoolExecutor(max_workers=8)`)
- [x] Объединённый endpoint `/api/dashboard/full`
- [x] Retry для ЦБ РФ через `tenacity`
- [x] APScheduler price_update: 15 → 30 мин
- [x] `DEFAULT_PAGE_SIZE`: 50 → 10

### 15.2 Купонная аналитика ✅
- [x] `get_coupon_calendar` cap: 12 → 60
- [x] `/api/portfolio/calendar` принимает `?limit` (до 1000) и `?days`
- [x] `calc_coupons_received(active, from, to)` — реальные купоны от `purchase_date`
- [x] `/api/portfolio/coupons_received?period=1w|1m|3m|6m|1y|all`
- [x] `/api/portfolio/yield?period=&mode=all|coupons` — купоны + realized + unrealized

### 15.3 Frontend ✅
- [x] Lazy-load `portfolio-extras.js` (watchlist + screener) через stub'ы на `window`
- [x] Per-page selector 10/20/50/100, `bondsPerPage` в localStorage
- [x] Debounce поиска 350 мс, минимум 3 символа
- [x] Кастомные модалки подтверждения вместо браузерного `confirm()`
- [x] Главная карточка «Доходность» с периодами и режимами (Все / Только купоны)
- [x] Карточка «Полученные купоны»
- [x] Sharpe Ratio ужат в компактный блок
- [x] Модалка купонов: до 1000 событий, клиентский фильтр 30/90/365/Все
- [x] Fix истории сделок (дубль `<td>` убран), фильтр дат в одну строку
- [x] RGBI benchmark: поддержка обоих форматов ответа

### 15.4 Tinkoff sync — баг-фиксы ✅
- [x] SSL bypass (`session.verify = False`) для self-signed cert в цепочке
- [x] `heal_amortized_buy_price`: ×10/×100 для нормализации T-Invest цен амортизируемых
- [x] `window.syncTinkoff` (была локальная)
- [x] Убран ложный тост «Сбой сети» после успешного sync

### 15.5 Infra ✅
- [x] Gunicorn 4×2 → 2×4 (`gthread`); RAM −200 МБ
- [x] Остановлены неиспользуемые host-сервисы (`postgresql@14-main`, `packagekit`, `multipathd`)
- [x] `vm.swappiness`: 60 → 10

### 15.6 Docs ✅
- [x] `docs/adr/0002-performance-caching-strategy.md`
- [x] `docs/architecture.md` обновлён (Redis-ключи)

---

## Этап 16 — Валюты и Золотые облигации (v2.6.0) ⏳ В ПЛАНЕ

> Цель: Поддержка валютных и золотых облигаций, валютной переоценки и парсинга PDF-отчетов.

### 16.1 Валютные облигации (CNY, USD, EUR)
...
### 16.3 Интеграция с брокерскими отчетами
...
Консолидированный список отложенных задач из прошлых этапов для планирования будущих релизов:

- [ ] **UI/UX**: `design-system.css` — отдельный файл с Chip/Tag/Tooltip (из этапа 0.1)
- [ ] **Аналитика**: Sparklines в колонке цены (из этапа 0.5)
- [ ] **Косметика**: Анимация `sun ↔ moon` при переключении темы (из этапа 0.7)
- [ ] **Тестирование**: Интеграционные тесты с реальной PostgreSQL через `testcontainers` (из этапа 5)
- [ ] **E2E тесты**: Автоматизация E2E-сценариев через Playwright (из этапа 5)
- [ ] **Покрытие**: Повышение покрытия автотестами до уровня ≥ 85% (из этапа 5)
- [ ] **Деплой**: Интеграция Let's Encrypt + certbot для авто-обновления SSL-сертификатов (из этапа 6.4)
